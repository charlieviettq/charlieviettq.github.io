...
excerpt: >
  ...
  reliability diagram, fit Platt/isotonic đúng split dữ liệu và monitor sau deploy.
...
- Khi PD (Probability of Default - xác suất vỡ nợ) đi vào pricing, hạn mức hoặc ECL (Expected Credit Loss - tổn thất tín dụng kỳ vọng), calibration trở thành vấn đề governance, tức một phần của quản lý rủi ro mô hình, không phải bước làm đẹp kết quả.
...
- Luôn kiểm tra trên test/OOT cuối cùng. OOT (Out-of-Time) là tập kiểm tra theo thời gian, thường dùng để xem model có giữ được chất lượng trên cohort mới hơn không.

Sơ đồ dưới đây tóm tắt ba pattern calibration hay gặp. Khi đọc hình, hãy nhìn vị trí đường cong so với đường chéo: nằm dưới nghĩa là model đang overconfident, tức dự báo rủi ro cao hơn thực tế; nằm trên nghĩa là model đang underconfident, tức dự báo rủi ro thấp hơn thực tế.
...
Nói đơn giản, nếu mô hình gán PD khoảng 8% cho một nhóm khách, thì sau khi cohort đã đủ thời gian để rủi ro xuất hiện, bad rate thực tế của nhóm đó cũng nên quanh 8%. Nếu thực tế chỉ 3% hoặc lên 15%, con số PD đang không đáng tin.
...
Lỗi tôi hay thấy khi review model là team chọn mô hình bằng AUC/Gini, sau đó dùng thẳng output probability cho pricing, hạn mức hoặc ECL. Về mặt kỹ thuật, output probability chỉ là xác suất model trả ra; nó có thể tốt để xếp hạng, nhưng chưa chắc đã là một xác suất đáng tin.

Hậu quả không nhỏ:

- **Risk-based pricing:** PD bị thổi phồng làm lãi suất bị đẩy lên, dễ tạo adverse selection, tức khách tốt rời đi còn khách rủi ro cao ở lại.
- **Credit limit:** hạn mức có thể bị siết quá mạnh hoặc nới quá rộng.
- **ECL:** công thức `ECL = PD × LGD × EAD` sẽ sai nếu PD sai scale.
- **Báo cáo danh mục:** mean predicted PD, tức PD dự báo trung bình, không khớp với bad rate thực tế.
...
Bảng dưới đây giúp bạn dự đoán vì sao một số nhóm model thường bị lệch calibration. Đây không phải lỗi code; đó là tính chất của cách model học score.

| Nhóm model | Xu hướng | Ghi chú |
|--------------|----------|---------|
...
`scale_pos_weight` / class weight trong training **không thay thế calibration**. Chúng chỉ giúp cân bằng class trong loss, không tự sửa thang xác suất trên population thật.
...
Để vẽ đường cong đầy đủ, ta thường chia test set thành các bin theo PD. Bin ở đây là nhóm hồ sơ có mức PD gần nhau. Với credit scoring, equal-frequency bin, tức mỗi bin có số hồ sơ gần bằng nhau, thường dễ đọc hơn.
...
6. Monitor mean PD vs actual sau khi label đủ chín.

Đoạn code dưới đây minh họa cách tách train, calibration và test. Điểm quan trọng là calibration set không được dùng cho early stopping, feature selection hoặc hyperparameter tuning; nếu đã dùng để chọn model, nó không còn là tập “sạch” để hiệu chỉnh xác suất.
...
X_train, X_calib, y_train, y_calib = train_test_split(
    X_trainval,
    y_trainval,
    test_size=0.25,  # 25% của trainval = 20% tổng dữ liệu
    stratify=y_trainval,
    random_state=42,
)
```

Sau bước này, `X_calib` chỉ dùng để fit calibration. Nếu nó đã tham gia tuning trước đó, calibration plot có thể đẹp trên báo cáo nhưng dễ fail khi deploy.

Với boosting model, tôi thường bắt đầu bằng Platt scaling vì đơn giản và ít overfit hơn isotonic regression, một cách hiệu chỉnh linh hoạt hơn nhưng cần nhiều mẫu bad hơn để ổn định.
...
calibrated = CalibratedClassifierCV(
    ...
    cv="prefit",       # model đã train; chỉ fit lớp calibration
)
...
| Chỉ check tổng thể | Toàn danh mục tốt, từng nhóm xấu | PD lệch ở NTB/ETB, product hoặc channel | Vẽ reliability theo từng lát cắt |
| Calibrate trên downsampled data | PD bị phóng đại hoặc kéo thấp | Pricing/ECL sai scale | Calibrate trên population rate hoặc correct prior |
...
- PD có dùng cho pricing, limit, ECL hoặc báo cáo danh mục không?
- Calibration set có thật sự unseen, tức chưa từng dùng để chọn model, không?
...
**Có cần calibration theo từng nhóm không?**  
Có thể cần. Nếu NTB/ETB, product hoặc channel có reliability curve khác nhau, calibration tổng thể là chưa đủ.
